...
- **Documentation Maintenance:** Whenever you modify JPA entities or the database schema, you MUST update the `DB.md` file to keep the ERD (Entity Relationship Diagram) and table descriptions perfectly synchronized with the codebase.
...
- **Architecture:** Standard Layered Architecture.
  ...
  - `entity`: JPA entities (`Account` implements `UserDetails`, `Role` enum, `Asset`, `AssetDailyValue`, `AssetSource`, `AccountFavoriteAsset`, `Alert`, `TriggeredAlert`, `AlertType` enum, `AlertDirection` enum, `ChartPattern`, `ChartPatternType` enum, `ChartPatternCategory` enum, `AssetPrediction`, `CommunitySentiment`, `SentimentType` enum).
  - `dto`: Data Transfer Objects (`RegisterRequest`, `LoginRequest`, `AuthResponse`, `AssetSummaryResponse`, `CandleResponse`, `CreateAlertRequest`, `UpdateAlertRequest`, `AlertResponse`, `TriggeredAlertResponse`, `ChartPatternResponse`, `AiPredictionResponse`, `AiHealthResponse`, `AssetPredictionResponse`, `PredictionFeaturesDto`, `PredictionStatsDto`, `AssetBacktestResultDto`, `GlobalBacktestStatsDto`, `SentimentRequest`, `SentimentResponse`, `SentimentPollResponse`).
...
- **Community Sentiment API:** Implemented. `PUT /assets/{symbol}/sentiments/me` allows a user to vote or update their general prediction (`BULLISH` or `BEARISH`) on an asset. `GET /assets/{symbol}/sentiments/poll` returns the aggregated results across all users. `GET /assets/{symbol}/sentiments/me` returns the current user's vote. Prevent duplicates via unique constraint `(account_id, asset_id)`.
- **AI Prediction API:** Implemented. `GET /predictions/health` checks the status of the external AI service. `GET /predictions/top` returns the top absolute predicted variations for a given date. `GET /predictions/{symbol}` returns the historical predictions for a specific asset. `GET /predictions/stats` returns statistical data (min, max, mean, median, count) on all historical predictions to detect outliers. `GET /predictions/backtest/stats` returns global backtest success rates (both strict Close-to-Close and Max Potential High/Low accuracy) and absolute error metrics. `GET /predictions/backtest/assets` returns backtest metrics grouped by asset. `GET /predictions/backtest/assets/{symbol}` returns backtest metrics for a specific asset. Predictions are generated asynchronously during the nightly data sync (`AssetDataSyncService.syncForDate()`). Historical predictions are also fully back-evaluated during the initial data seed to provide instant accuracy metrics using a dual-evaluation system (`successRatePct` vs `maxPotentialSuccessRatePct`). For each asset, `AssetPredictionService` fetches up to 60 days of historical data, `PredictionFeatureService` calculates 23 technical features, and `AiPredictionClient` sends them to an external AI API via Spring `RestClient`. The resulting prediction is saved in the database with a unique constraint `(asset_id, date)`.
...
# 19. Get global backtest statistics
curl -s http://localhost:8080/predictions/backtest/stats

# 20. Get backtest statistics grouped by asset
curl -s http://localhost:8080/predictions/backtest/assets

# 21. Get backtest statistics for a specific asset
curl -s http://localhost:8080/predictions/backtest/assets/BTC


# 22. Submit a BULLISH sentiment on BTC
curl -s -X PUT http://localhost:8080/assets/BTC/sentiments/me \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"type":"BULLISH"}'

# 23. Get the community poll results for BTC
curl -s http://localhost:8080/assets/BTC/sentiments/poll
```
